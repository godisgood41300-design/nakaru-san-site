...
async function api(path, options = {}) {
  ...
  try {
    ...
    console.warn("Nakaru-san API fallback active", error);
...
async function syncPublicMessages() {
  try {
    ...
    console.error("Public chat sync failed", error);
    showToast("Connection issue", "Public chat could not reach the Nakaru-san API. Check Vercel environment variables and /api/status.", { dedupeMs: 30000 });
  }
}
...
async function syncDmMessages() {
  try {
    ...
    console.error("Direct message sync failed", error);
    showToast("Connection issue", "Direct messages could not reach the Nakaru-san API. Check Vercel environment variables and /api/status.", { dedupeMs: 30000 });
  }
}
...
document.querySelectorAll("[data-provider]").forEach((button) => {
  button.addEventListener("click", async () => {
    ...
    try {
      ...
      const providerStatus = status.oauth?.[provider];
      if (providerStatus && !providerStatus.configured) {
        const missing = providerStatus.missing.join(", ");
        authMessage.textContent = `${provider} login needs ${missing} in Vercel first.`;
        showToast("Social login setup needed", `Add ${missing} in Vercel before using ${provider} login.`);
        return;
      }
    } catch {
      authMessage.textContent = "Social login needs provider credentials in Vercel before launch.";
    }
