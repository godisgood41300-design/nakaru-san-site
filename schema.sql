...
async function postYouTube(event) {
  ...
  render();
  try {
    await savePost({ type: "youtube", content: "Shared a YouTube video.", youtube_url: parsed.originalUrl, youtube_embed_url: parsed.embedUrl });
    state.youtubeStatus = "Video posted to the live feed.";
...
function authView() {
  return `
    <section class="auth-card panel">
      ...
      <div class="oauth-row"><button onclick="social('google')" type="button">Google</button><button onclick="social('github')" type="button">GitHub</button><button onclick="social('facebook')" type="button">Facebook</button></div>
...
function renderPage() {
  ...
  if (state.page === "video") return `
    <main class="page-grid"><section class="panel video-post-panel"><div class="panel-title"><span class="eyebrow">Video link post</span><h2>Post a YouTube link</h2></div>
      ${state.videoComposerOpen ? `<form class="form-grid" onsubmit="postYouTube(event)"><label>YouTube URL<input placeholder="https://www.youtube.com/watch?v=..." /></label><button class="primary-action" ${state.videoPosting ? "disabled" : ""} type="submit">${state.videoPosting ? "Posting..." : "Post Video Link"}</button></form>` : `<div class="post-complete"><h3>Video posted.</h3><p>Your YouTube link is now saved as a feed post and connected to your account profile.</p><div class="hero-actions"><button class="primary-action" onclick="setPage('feed')" type="button">View Live Feed</button><button class="ghost-action" onclick="state.videoComposerOpen=true; state.youtubeStatus=''; render()" type="button">Post Another Video Link</button></div></div>`}
