...
function serviceBaseUrl(request) {
  ...
  return hostHeader ? `${protoHeader}://${hostHeader}` : "https://nakarusan.nakarusan.com";
}

function oauthRedirectUri(provider, request) {
  const baseUrl = serviceBaseUrl(request);
...
async function exchangeOAuthCode(provider, code, stateRecord, request) {
...
function handleApi(request, response, url) {
  if (url.pathname === "/api/status" && request.method === "GET") {
    const oauth = Object.fromEntries(
      Object.entries(oauthProviders).map(([provider, config]) => {
        const required = provider === "twitter"
          ? ["X_CLIENT_ID"]
          : [`${provider.toUpperCase()}_CLIENT_ID`, `${provider.toUpperCase()}_CLIENT_SECRET`];
        const missing = required.filter((key) => !process.env[key] && !(provider === "twitter" && key === "X_CLIENT_ID" && process.env.TWITTER_CLIENT_ID));
        return [provider, {
          configured: missing.length === 0,
          missing,
          redirectUri: oauthRedirectUri(provider, request)
        }];
      })
    );
    json(response, 200, {
      ok: true,
      appBaseUrl: serviceBaseUrl(request),
  ...
  if (url.pathname.match(/^\/api\/auth\/oauth\/[^/]+$/) && request.method === "GET") {
    ...
    const params = new URLSearchParams({
      client_id: config.clientId,
      redirect_uri: oauthRedirectUri(provider, request),
      response_type: "code",
      scope: config.scope,
      state
    });
  ...
  if (url.pathname.match(/^\/api\/auth\/oauth\/[^/]+\/callback$/) && request.method === "GET") {
    ...
    exchangeOAuthCode(provider, code, stateRecord, request)
      .then((account) => {
        const sessionId = randomUUID();
        sessions.set(sessionId, account.id);
        response.setHeader("Set-Cookie", `nakaru_session=${sessionId}; HttpOnly; SameSite=Lax; Path=/; Max-Age=2592000`);
  ...
  if (url.pathname === "/api/feed" && request.method === "POST") {
    readBody(request)
      .then(({ from = "YukiKaze", text = "", content = "", image = "", youtubeUrl = "", youtubeEmbedUrl = "", postType = "" }) => {
        const cleanText = (text || content || (youtubeUrl ? "Shared a YouTube video." : "")).trim();
        const cleanYouTubeUrl = youtubeUrl || extractYouTubeUrl(cleanText);
        const embedUrl = youtubeEmbedUrl || getYouTubeEmbedUrl(cleanYouTubeUrl || cleanText);
        if (!cleanText && !image && !embedUrl) return json(response, 400, { error: "Post cannot be empty" });
        const appropriate = isAppropriate(cleanText);
        const post = {
          id: randomUUID(),
          from,
          text: cleanText,
          content: cleanText,
          postType: postType || (embedUrl ? "youtube" : image ? "image" : "text"),
          post_type: postType || (embedUrl ? "youtube" : image ? "image" : "text"),
