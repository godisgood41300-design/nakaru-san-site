...
export default async function handler(req, res) {
  const requestUrl = new URL(req.url || "/", `https://${req.headers.host || "nakaru-san.com"}`);
  ...
  try {
    ...
    if (route.match(/^\/auth\/oauth\/[^/]+$/) && method === "GET") {
      ...
      if (!config) {
        return res.status(501).json({
          error: "Social login is not enabled yet.",
          provider
        });
      }
    ...
    if (route.match(/^\/auth\/oauth\/[^/]+\/callback$/) && method === "GET") {
      ...
      if (!oauthProviders[provider]) {
        return res.status(501).send("<h1>Social sign-in is not enabled yet.</h1><p>Please use email login for now.</p>");
      }
    ...
    if (route === "/feed" && method === "POST") {
      const { from = "YukiKaze", text = "", content = "", image = "", youtubeUrl = "", youtubeEmbedUrl = "", postType = "" } = parseBody(req);
      ...
      if (!cleanText && !image && !embedUrl) return res.status(400).json({ error: "Post cannot be empty" });
      ...
      const post = {
        ...
        text: cleanText,
        content: cleanText,
        postType: postType || (embedUrl ? "youtube" : uploadedImage ? "image" : "text"),
        post_type: postType || (embedUrl ? "youtube" : uploadedImage ? "image" : "text"),
