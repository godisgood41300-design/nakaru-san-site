const rooms = [
  ...
  {
    title: "Creator Dojo",
    theme: "Original characters, art drops, and lore talk",
    viewers: "0",
    tags: ["OCs", "Art", "Feedback"],
    category: "Creators",
    gradient: "linear-gradient(145deg, #0f0f18, #8f49ff 48%, #38bdf8)"
  },
  {
    title: "Late Night Queue",
    theme: "Cozy watchlists, co-op lobbies, and after-hours chat",
    viewers: "0",
    tags: ["Cozy", "Gaming", "Open mic"],
    category: "Community",
    gradient: "linear-gradient(145deg, #07060a, #6f35e8 54%, #f4c95d)"
  }
];
...
const profileEyebrow = document.querySelector("#profilePage .profile-details .eyebrow");
const profileStatus = document.querySelector("#profileStatus");
const ownerControls = document.querySelectorAll("[data-owner-controls]");
let profile = JSON.parse(localStorage.getItem("nakaruProfile") || "{}");
let profileEditing = false;

const APP_VERSION = "20260521-community-polish";
...
async function supabaseApi(path, options = {}) {
  ...
  if (route === "/messages" && method === "POST") {
    ...
    const message = { id: crypto.randomUUID(), room: body.room || "Moonlit Lounge", from: body.from || displayName(), text, at: Date.now() };
  ...
  if (route === "/dm" && method === "POST") {
    const text = String(body.text || "").trim();
    if (!text) throw new Error("Message cannot be empty.");
    const message = { id: crypto.randomUUID(), thread: body.thread || "RaeArcade", from: body.from || displayName(), text, at: Date.now() };
...
async function localApi(path, options = {}) {
  ...
  if (route === "/feed" && method === "POST") {
    ...
    const embedUrl = body.youtubeEmbedUrl || (body.youtubeUrl ? getYouTubeEmbedUrl(body.youtubeUrl) : getYouTubeEmbedUrl(content));
    const youtubeUrl = body.youtubeUrl || getYouTubeUrlFromText(content);
    const post = localPostShape({
      ...body,
      user_id: currentAccount?.id || "",
      from: currentAccount?.username || displayName(),
      content,
  ...
  if (route === "/messages" && method === "POST") {
    ...
    const messages = localRead("nakaruLocalMessages", localSeedMessages);
    messages[room] ||= [];
    const message = { id: crypto.randomUUID(), from: body.from || displayName(), text, at: Date.now() };
  ...
  if (route === "/presence") {
    const id = userId || localStorage.getItem("nakaruUserId") || crypto.randomUUID();
    return {
      userId: id,
      total: 1,
      room: activeRoom,
      roomCount: 1,
      users: [{ id, name: displayName(), room: activeRoom }]
    };
  }
...
function renderRooms(items) {
  roomList.innerHTML = items
    .map(
      (room) => `
        <article class="room-card ${room.title === activeRoom ? "active" : ""}">
          <div class="thumb" style="--room-bg: ${room.gradient}">
            <span>${escapeHtml(room.category || "Room")}</span>
          </div>
          <div class="room-info">
            <strong>${escapeHtml(room.title)}</strong>
            <div class="room-meta">
              <span>${escapeHtml(room.theme)}</span>
              <span>${escapeHtml(room.viewers)} online now</span>
            </div>
            <div class="room-tags">
              ${room.tags.map((tag) => `<span>${escapeHtml(tag)}</span>`).join("")}
            </div>
          </div>
          <button class="join-button ${room.title === activeRoom ? "active" : ""}" data-room="${escapeHtml(room.title)}" type="button">
            ${room.title === activeRoom ? "Joined" : "Join Room"}
          </button>
        </article>
      `
    )
...
function renderMessages(items, target) {
  ...
  if (!safeItems.length) {
    target.innerHTML = `<p class="empty-note">No messages in this room yet. Start the conversation.</p>`;
    return;
  }
  target.innerHTML = safeItems
    .map(({ from = "Nakaru Member", text = "", at }) => {
      const initials = from.slice(0, 2).toUpperCase();
      const avatarStyle = from === displayName() && profile.photo
        ? ` style="background-image:url('${escapeHtml(profile.photo)}');background-size:cover;background-position:center;color:transparent"`
        : "";
      const stamp = Number(at) || Date.parse(at);
      const time = Number.isFinite(stamp) ? new Date(stamp).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }) : "";
      return `
        <article class="message">
          <div class="avatar"${avatarStyle}>${escapeHtml(initials)}</div>
          <div>
            <strong>${escapeHtml(from)} ${time ? `<small>${escapeHtml(time)}</small>` : ""}</strong>
...
async function postYouTubeVideo() {
  ...
  youtubeInput.value = "";
  watchStatus.textContent = "Link posted";
  if (data.visible) {
    feedPosts = [data.post, ...feedPosts.filter((post) => post.id !== data.post.id)];
    renderFeedPosts(feedPosts);
    feedStatus.textContent = `${feedPosts.length} posts`;
    showToast("Video link posted", "The YouTube link is now on the Nakaru-san home feed and your profile.");
  } else {
...
function setAuthMode(mode) {
  ...
  authMessage.textContent = "Social sign in opens through Supabase when Google, Facebook, GitHub, or X is enabled.";
}

function setProfileEditing(isEditing) {
  const canEdit = Boolean(currentAccount);
  profileEditing = Boolean(isEditing && canEdit);
  profilePanel?.classList.toggle("is-editing", profileEditing);
  ownerControls.forEach((control) => {
    control.hidden = !canEdit;
    control.setAttribute("aria-hidden", String(!canEdit));
  });
  if (saveProfileButton && profileEditing) {
    saveProfileButton.innerHTML = `<span class="button-icon icon-save" aria-hidden="true"></span>Update Profile`;
  }
  if (saveProfileButton) saveProfileButton.hidden = !canEdit || !profileEditing;
  if (editProfileButton) editProfileButton.hidden = !canEdit || profileEditing;
  if (profileEditor) profileEditor.setAttribute("aria-hidden", String(!profileEditing));
  if (profileStatus && profileEditing) profileStatus.textContent = "";
}

function updateAccountUi() {
  const isSignedIn = Boolean(currentAccount);
  accountButton.textContent = currentAccount ? currentAccount.username : "Sign In";
  accountButton.classList.toggle("is-authenticated", isSignedIn);
  document.body.classList.toggle("is-signed-in", isSignedIn);
  document.body.classList.toggle("is-public-view", !isSignedIn);
  ...
  if (!isSignedIn) profileEditing = false;
...
function renderProfile() {
  ...
  if (profileEyebrow) profileEyebrow.textContent = currentAccount ? "Your profile" : "Community profile";
...
async function saveProfile(event) {
  ...
  await withButtonPending(saveProfileButton, "Updating...", async () => {
    try {
      const user = await getCurrentSupabaseUser();
      if (!user) {
        if (profileStatus) profileStatus.textContent = "Please log in before updating your profile.";
      ...
      showToast("Profile updated", "Profile updated.");
    } catch (error) {
      console.error("Profile save failed", error);
      if (profileStatus) profileStatus.textContent = "Profile could not be updated. Please try again soon.";
...
messageForm.addEventListener("submit", async (event) => {
  ...
  if (!text) {
    messageInput.focus();
    activeRoomStatus.textContent = "Enter a message";
    return;
  }
  await withButtonPending(button, "Sending...", async () => {
    try {
      activeRoomStatus.textContent = "Sending...";
      await chatApi("/api/messages", { method: "POST", body: JSON.stringify({ room: activeRoom, from: displayName(), text }) });
      messageInput.value = "";
      await syncPublicMessages();
      console.info("Nakaru-san room message sent", { room: activeRoom });
    } catch (error) {
...
dmForm.addEventListener("submit", async (event) => {
  ...
  const button = dmForm.querySelector("button[type='submit']");
  if (!text) {
    dmInput.focus();
    activeDmStatus.textContent = "Enter a message";
    return;
  }
  await withButtonPending(button, "Sending...", async () => {
    try {
      activeDmStatus.textContent = "Sending...";
      await chatApi("/api/dm", { method: "POST", body: JSON.stringify({ thread: activeThread, from: displayName(), text }) });
      dmInput.value = "";
      await syncDmMessages();
      console.info("Nakaru-san direct message sent", { thread: activeThread });
    } catch (error) {
      console.error("Direct message send failed", error);
      activeDmStatus.textContent = "Messages unavailable";
      showSafeChatError();
    }
  });
});
