...
const kanjiWordRainItems = [
  ["絆", 4, 18, 0, "purple"], ["NAKARU", 12, 23, 6, "gold word"], ["光", 20, 17, 12, "purple"], ["SAN", 28, 28, 3, "purple word"],
  ["武", 36, 19, 9, "purple"], ["影", 44, 25, 15, "purple"], ["NAKARU", 52, 18, 5, "gold word"], ["月", 60, 24, 11, "purple"],
  ["火", 68, 16, 2, "gold"], ["SAN", 76, 27, 8, "purple word"], ["道", 84, 20, 14, "gold"], ["和", 92, 26, 4, "purple"],
  ["NAKARU", 8, 31, 17, "gold word"], ["夢", 32, 22, 20, "purple"], ["光", 57, 29, 22, "gold"], ["SAN", 88, 18, 19, "purple word"]
];
...
const state = {
  ...
  likedPosts: readLocal("nakaru-liked-posts", []),
  commentingPostId: "",
  commentDrafts: {},
  postComments: {},
  postActionStatus: "",
...
async function createFriendshipRows(friendId) {
  if (!supabaseClient || !state.user || !friendId || friendId === state.user.id) return false;
  const rows = [
    { user_id: state.user.id, friend_id: friendId },
    { user_id: friendId, friend_id: state.user.id }
  ];
...
function relationTo(userId) {
  ...
  if ([...state.incomingRequests, ...state.outgoingRequests].some((request) => [request.sender_id, request.receiver_id].includes(userId) && request.status === "accepted")) return "friend";
...
function notificationText(type, fallback) {
  const map = {
    ...
    "audio-call": "Incoming audio call",
    live: "Live room invite",
    "live-invite": "Live room invite"
  };
  return map[type] || fallback || "Notification";
}

function addNotification(type, text, refId = "") {
  const item = { id: crypto.randomUUID(), type, text: text || notificationText(type), refId, created_at: new Date().toISOString(), read: false };
  state.notifications = [
    item,
    ...state.notifications
  ].slice(0, 20);
...
function notificationTarget(type) {
  if (type === "request" || type === "accepted") return "friend-requests";
  if (type === "message") return "messages";
  if (type === "call" || type === "video-call" || type === "audio-call") return "calls";
  if (type === "live" || type === "live-invite") return "live";
  return "home";
}
...
function notificationTitle(type) {
  const map = {
    request: "New friend request",
    accepted: "Friend request accepted",
    message: "New Nakaru-San message",
    call: "Incoming Nakaru-San call",
    "video-call": "Incoming video call",
    "audio-call": "Incoming audio call",
    live: "Live room invite",
    "live-invite": "Live room invite"
...
async function showDeviceNotification(item) {
  ...
  try {
    ...
    const urgent = isUrgentNotification(item.type);
    const options = {
      body: item.text,
    ...
    if (registration?.showNotification) {
      registration.showNotification(notificationTitle(item.type), options);
    } else {
      new Notification(notificationTitle(item.type), options);
    }
  } catch (error) {
...
async function savePushSubscription(registration) {
  if (!supabaseClient || !state.user || !config.vapidPublicKey || !registration?.pushManager) return;
...
async function enableNotifications() {
  if (state.notificationSaving) return;
  state.notificationSaving = true;
  state.notificationStatus = "Requesting notification permission...";
  render();
  try {
    if (!("Notification" in window)) {
      state.notificationStatus = "This browser does not support notifications.";
      return;
    }
    const permission = await Notification.requestPermission();
    state.notificationPermission = permission;
    if (permission !== "granted") {
      state.notificationStatus = "Notifications were not enabled. You can allow them in your browser settings.";
      return;
    }
    const registration = "serviceWorker" in navigator ? await navigator.serviceWorker.ready : null;
    ...
    showDeviceNotification({ id: "test", type: "message", text: "Nakaru-San notifications are ready.", refId: "" });
  } catch (error) {
    console.error("Notification setup failed", error);
    state.notificationStatus = "Notifications could not be enabled on this device.";
  } finally {
...
function openNotification(type) {
  ...
  setPage(notificationTarget(type));
}
...
function ensureMusicGraph() {
  ...
  if (!AudioContextClass) {
    state.musicStatus = "This browser does not support background music.";
    return null;
  }
  if (!musicAudioContext) {
    musicAudioContext = new AudioContextClass();
    musicGainNode = musicAudioContext.createGain();
    musicDelayNode = musicAudioContext.createDelay(2.4);
    ...
    musicFilterNode.frequency.value = 3200;
    musicFilterNode.Q.value = 0.6;

    musicDelayNode.connect(musicFeedbackNode);
    musicFeedbackNode.connect(musicDelayNode);
    musicDelayNode.connect(musicFilterNode);
...
function playOriginalFluteNote(frequency, startTime, duration, level = 0.05) {
  if (!musicAudioContext || !musicDelayNode) return;
  const oscillator = musicAudioContext.createOscillator();
  const overtone = musicAudioContext.createOscillator();
  const noteGain = musicAudioContext.createGain();
  const toneFilter = musicAudioContext.createBiquadFilter();
  ...
  toneFilter.frequency.setValueAtTime(frequency * 2.2, startTime);
  toneFilter.Q.setValueAtTime(2.1, startTime);
  ...
  noteGain.connect(musicDelayNode);
...
function scheduleOriginalAnimeAmbience() {
  if (!musicAudioContext || !state.musicEnabled) return;
  const scale = [293.66, 329.63, 392.0, 440.0, 493.88, 587.33, 659.25];
  const phrase = [0, 2, 3, 5, 3, 2, 1, 0, 4, 5, 3, 2];
  const now = musicAudioContext.currentTime;
  if (!musicNextTime || musicNextTime < now + 0.2) musicNextTime = now + 0.25;

  while (musicNextTime < now + 8) {
    const noteIndex = phrase[musicStep % phrase.length];
...
async function startBackgroundMusic() {
  const context = ensureMusicGraph();
  if (!context) {
    render();
    return;
  }
  try {
    ...
    musicGainNode.gain.setTargetAtTime(0.035, now, 0.35);
    ...
    state.musicStatus = "Music could not start. Tap again or check browser audio permissions.";
  }
  render();
}
...
function stopBackgroundMusic() {
  state.musicEnabled = false;
  state.musicStatus = "Background music paused.";
  clearInterval(musicTimer);
  musicTimer = null;
  musicNextTime = 0;
  if (musicGainNode && musicAudioContext) {
    const now = musicAudioContext.currentTime;
    musicGainNode.gain.cancelScheduledValues(now);
    musicGainNode.gain.setTargetAtTime(0.0001, now, 0.25);
  }
  render();
}
...
function drawDragonBackground(time = 0) {
  ...
  for (let index = 0; index < starCount; index += 1) {
    ...
    const size = 0.7 + (index % 5) * 0.38 + starPulse * 0.45;
    ctx.fillStyle = index % 3 === 0 ? "rgba(96, 165, 250, 0.42)" : "rgba(186, 230, 253, 0.34)";
    ctx.shadowColor = "rgba(56, 189, 248, 0.42)";
    ctx.shadowBlur = 10 + size * 4;
    ctx.beginPath();
  ...
  points.forEach((point, index) => {
    if (index % 2 !== 0) return;
    const scale = 1 - point.progress * 0.55;
    const next = points[Math.min(index + 1, points.length - 1)] || point;
    const angle = Math.atan2(next.y - point.y, next.x - point.x);
    ctx.save();
    ctx.translate(point.x, point.y);
    ctx.rotate(angle + Math.PI / 2);
    ctx.strokeStyle = index % 4 === 0 ? "rgba(250, 204, 21, 0.28)" : "rgba(168, 85, 247, 0.3)";
    ctx.lineWidth = Math.max(1.2, 3.1 * scale);
    ctx.beginPath();
  ...
  points.forEach((point, index) => {
    if (index % 3 !== 0 || index < 4) return;
    const size = 8 + (1 - point.progress) * 16;
    const next = points[Math.min(index + 1, points.length - 1)] || point;
    const angle = Math.atan2(next.y - point.y, next.x - point.x);
    ctx.save();
    ctx.translate(point.x, point.y);
    ctx.rotate(angle - Math.PI / 2);
    ctx.fillStyle = index % 6 === 0 ? "rgba(250, 204, 21, 0.24)" : "rgba(124, 60, 255, 0.26)";
...
function mergePublicProfiles(profiles = []) {
  const map = new Map(state.publicProfiles.map((profile) => [profile.id, profile]));
  for (const profile of profiles) {
    if (profile?.id) map.set(profile.id, { ...(map.get(profile.id) || {}), ...profile });
  }
  state.publicProfiles = [...map.values()];
}

async function loadProfilesByIds(ids = []) {
  const uniqueIds = [...new Set(ids.filter(Boolean))];
  if (!supabaseClient || !uniqueIds.length) return [];
...
async function loadPosts() {
  ...
  const { data, error } = await supabaseClient
    .from("posts")
    .select("id,user_id,content,post_type,media_url,youtube_url,youtube_embed_url,likes,comments_count,created_at,updated_at")
...
async function loadFriendRequests() {
  ...
  await loadProfilesByIds(requests.flatMap((request) => [request.sender_id, request.receiver_id]));
}
...
async function loadFriendships() {
  ...
  await loadProfilesByIds(friendIds);
  state.friends = friendIds.map(profileById).filter((profile) => profile.id);
  const activeRecipientIsFriend = state.activeDmRecipient && friendIds.includes(state.activeDmRecipient);
  if (!activeRecipientIsFriend) {
    state.activeDmRecipient = state.friends[0]?.id || "";
    state.activeConversationId = "";
    state.activeConversationPairKey = "";
    state.dmMessages = [];
  }
...
async function loadLiveRooms() {
  ...
  state.liveRoomSearchResults = query
    ...
    : state.liveRooms;
  if (state.activeLiveRoom && !state.liveRooms.some((room) => room.id === state.activeLiveRoom?.id)) {
    state.activeLiveRoom = null;
  }
}
...
async function ensureConversation(friendId) {
  ...
  if (state.activeConversationId && state.activeConversationPairKey === pairKey) return state.activeConversationId;
  let conversation = null;
  let conversationKeySupported = true;

  let { data, error } = await supabaseClient
    .from("conversations")
    .select("id,participant_ids,conversation_key,created_at,updated_at")
    .eq("conversation_key", pairKey)
    .maybeSingle();
  if (error && isMissingColumnError(error, "conversation_key")) {
    conversationKeySupported = false;
  } else if (error) {
    throw error;
  } else {
    conversation = data;
  ...
  if (!conversation) {
    ({ data, error } = await supabaseClient
      ...
      .limit(20));
    if (error) throw error;
    conversation = (data || []).find((row) => {
      const ids = Array.isArray(row.participant_ids) ? row.participant_ids.map(String).sort() : [];
      return ids.length === 2 && ids[0] === participants[0] && ids[1] === participants[1];
    });
  }

  if (!conversation) {
    const insertRow = conversationKeySupported ? { participant_ids: participants, conversation_key: pairKey } : { participant_ids: participants };
    ({ data, error } = await supabaseClient
      .from("conversations")
      .insert(insertRow)
      .select(conversationKeySupported ? "id,participant_ids,conversation_key,created_at,updated_at" : "id,participant_ids,created_at,updated_at")
      .single());
    if (error && conversationKeySupported && isMissingColumnError(error, "conversation_key")) {
      ({ data, error } = await supabaseClient
        .from("conversations")
  ...
  state.activeConversationPairKey = pairKey;
...
function subscribeSocialRealtime() {
  ...
  state.socialChannel = supabaseClient
    ...
    .on("postgres_changes", { event: "*", schema: "public", table: "friend_requests" }, async (payload) => {
      ...
      if (row.receiver_id === state.user.id && row.status === "pending") {
        addNotification("request", "You have a new friend request.", row.id);
      }
      if (row.sender_id === state.user.id && row.status === "accepted") {
        addNotification("accepted", "Your friend request was accepted.", row.id);
      }
    ...
    .on("postgres_changes", { event: "INSERT", schema: "public", table: "direct_messages" }, async (payload) => {
      const row = payload.new || {};
      if (![row.sender_id, row.recipient_id].includes(state.user.id)) return;
      if (row.sender_id !== state.user.id) addNotification("message", "You have a new direct message.", row.id);
      if (row.recipient_id === state.user.id || row.sender_id === state.activeDmRecipient) await loadDirectMessages();
      render();
    })
    .on("postgres_changes", { event: "*", schema: "public", table: "calls" }, async (payload) => {
      ...
      if (row.receiver_id === state.user.id && row.status === "ringing") {
        const isAudio = row.call_type === "audio";
        addNotification(isAudio ? "audio-call" : "video-call", `Incoming ${isAudio ? "audio" : "video"} call.`, row.id);
      }
      if (row.caller_id === state.user.id && row.status === "accepted") {
        addNotification("call", `Your ${row.call_type === "audio" ? "audio" : "video"} call was accepted.`, row.id);
      }
      await loadCalls();
      render();
    })
    ...
    .on("postgres_changes", { event: "*", schema: "public", table: "live_room_invites" }, async (payload) => {
      const row = payload.new || payload.old || {};
      if (![row.sender_id, row.receiver_id].includes(state.user.id)) return;
      if (row.receiver_id === state.user.id && row.status === "pending") addNotification("live-invite", "You were invited to join a live room.", row.id);
      if (row.sender_id === state.user.id && row.status === "accepted") addNotification("live-invite", "Your live room invite was accepted.", row.id);
...
function setPage(page) {
  ...
  if (page === "friend-requests") {
    Promise.allSettled([loadPublicProfiles(), loadFriendRequests(), loadFriendships()]).finally(render);
  }
  if (page === "messages" || page === "inbox") {
    Promise.allSettled([loadPublicProfiles(), loadFriendships()])
...
async function sendFriendRequest(receiverId) {
  ...
  try {
    ...
    if (existing?.status === "accepted") {
      await createFriendshipRows(receiverId);
      state.socialStatus = "You are friends now. Messaging is available.";
      state.activeDmRecipient = receiverId;
      state.activeConversationId = "";
      state.activeConversationPairKey = "";
      state.dmMessages = [];
      await Promise.allSettled([loadFriendRequests(), loadFriendships(), loadPublicProfiles()]);
      render();
      return;
    }
...
async function respondFriendRequest(requestId, status) {
  ...
  try {
    ...
    if (error) throw error;
    if (status === "accepted") {
      await createFriendshipRows(request.sender_id);
      state.activeDmRecipient = request.sender_id;
      state.activeConversationId = "";
      state.activeConversationPairKey = "";
      state.dmMessages = [];
...
async function messageFriend(friendId) {
  const acceptedRequest = [...state.incomingRequests, ...state.outgoingRequests].find((request) => [request.sender_id, request.receiver_id].includes(friendId) && request.status === "accepted");
  if (!isFriend(friendId) && acceptedRequest) {
    try {
      await createFriendshipRows(friendId);
    } catch (error) {
      console.error("Friendship repair before messaging failed", error);
    }
  }
...
async function createTextPost(event) {
  event.preventDefault();
  if (state.postCreating) return;
  const form = event.currentTarget;
  const input = form.querySelector("input[name='postText'], textarea[name='postText'], input:not([type='file']), textarea");
  ...
  state.postStatus = file ? "Uploading media..." : "Posting...";
  render();
  try {
    let media = { media_url: "", media_type: "", media_name: "" };
    if (file) {
      media = await uploadChatMedia(file, `posts/${state.user.id}`);
    }
    const postType = media.media_type || "text";
    await savePost({
      post_type: postType,
      content: content || mediaLabel(media.media_type),
      media_url: media.media_url || null
    });
    if (input) input.value = "";
    if (fileInput) fileInput.value = "";
    state.postStatus = postType === "text" ? "Post shared." : `${mediaLabel(postType)} posted.`;
  } catch (error) {
    console.error("Post create failed", error);
    state.postStatus = "Post could not be shared. Try a smaller image/video or run the latest Supabase SQL.";
  } finally {
...
function updatePostInState(postId, updates) {
  state.posts = state.posts.map((post) => post.id === postId ? { ...post, ...updates } : post);
}
...
async function toggleLikePost(postId) {
  ...
  if (!post) return;
  if (!state.user) {
    state.postActionStatus = "Sign in to like posts.";
    render();
    return;
  }
  const liked = isPostLiked(postId);
  const nextLikes = Math.max(0, Number(post.likes || 0) + (liked ? -1 : 1));
  state.likedPosts = liked ? state.likedPosts.filter((id) => id !== postId) : [...state.likedPosts, postId];
  writeLocal("nakaru-liked-posts", state.likedPosts);
  updatePostInState(postId, { likes: nextLikes });
  state.postActionStatus = liked ? "Like removed." : "Post liked.";
  render();

  if (!supabaseClient) return;
  try {
    const { data, error } = await supabaseClient.rpc("set_post_like", { target_post_id: postId, should_like: !liked });
    if (error) throw error;
    if (typeof data === "number") updatePostInState(postId, { likes: data });
  } catch (error) {
    console.error("Like update failed", error);
    state.postActionStatus = "Like could not be saved. Run the latest Supabase SQL if this keeps happening.";
    render();
  }
}

async function toggleComments(postId) {
  state.commentingPostId = state.commentingPostId === postId ? "" : postId;
  state.postActionStatus = "";
  if (state.commentingPostId && supabaseClient) {
...
async function loadPostComments(postId) {
  ...
  const { data, error } = await supabaseClient
    ...
    .limit(80);
  if (error) {
    console.error("Post comments load failed", error);
    state.postActionStatus = "Comments could not load right now.";
    return;
  }
  state.postComments = { ...state.postComments, [postId]: data || [] };
  updatePostInState(postId, { comments_count: (data || []).length });
}

function updateCommentDraft(postId, value) {
  state.commentDrafts = { ...state.commentDrafts, [postId]: value };
}

async function submitPostComment(event, postId) {
  event.preventDefault();
  const content = String(state.commentDrafts[postId] || event.currentTarget.querySelector("input")?.value || "").trim();
  if (!content) return;
  if (!state.user) {
    state.postActionStatus = "Sign in to comment.";
    render();
    return;
  }
  const author = state.profile.display_name || state.profile.username || "Nakaru Member";
  const localComment = { id: crypto.randomUUID(), post_id: postId, user_id: state.user.id, author, content, created_at: new Date().toISOString() };
  try {
    let savedComment = localComment;
    if (supabaseClient) {
      const { data, error } = await supabaseClient
        .from("post_comments")
        .insert({ post_id: postId, user_id: state.user.id, author, content })
        .select("id,post_id,user_id,author,content,created_at")
        .single();
      if (error) throw error;
      savedComment = data;
    }
    const nextComments = [...(state.postComments[postId] || []), savedComment];
    state.postComments = { ...state.postComments, [postId]: nextComments };
    state.commentDrafts = { ...state.commentDrafts, [postId]: "" };
    updatePostInState(postId, { comments_count: nextComments.length });
    state.postActionStatus = "Comment posted.";
  } catch (error) {
    console.error("Comment failed", error);
    state.postActionStatus = "Comment could not be posted. Run the latest Supabase SQL if this keeps happening.";
  } finally {
    render();
  }
}
...
async function sharePost(postId) {
  ...
  const shareData = {
    title: "Nakaru-San post",
    text: post.content || "Check out this Nakaru-San post.",
    url: shareUrl
  };
  try {
    if (navigator.share) {
      await navigator.share(shareData);
      state.postActionStatus = "Share sheet opened.";
    } else if (navigator.clipboard?.writeText) {
      await navigator.clipboard.writeText(`${shareData.text}\n${shareUrl}`);
      state.postActionStatus = "Post link copied.";
    } else {
      state.postActionStatus = "Copy this link to share: " + shareUrl;
    }
  } catch (error) {
    console.warn("Share canceled or failed", error);
    state.postActionStatus = "Share was canceled.";
  } finally {
    render();
  }
}
...
async function setDmRecipient(profileId) {
  if (state.activeDmRecipient !== profileId) {
    state.activeConversationId = "";
    state.activeConversationPairKey = "";
    state.dmMessages = [];
  }
...
function updateRemoteParticipantsFromPresence() {
  const presenceState = state.callChannel?.presenceState?.() || {};
  const seen = new Set();
  const participants = [];
  for (const [key, entries] of Object.entries(presenceState)) {
    ...
    for (const entry of list) {
      ...
      if (!id || id === state.user?.id || seen.has(id)) continue;
      seen.add(id);
      participants.push({
        id,
        username: entry?.username || profileById(id).username || "member",
        display_name: entry?.display_name || profileName(profileById(id)),
        avatar_url: entry?.avatar_url || profileById(id).avatar_url || ""
      });
    }
  }
  for (const id of remoteStreams.keys()) {
    if (id === state.user?.id || seen.has(id)) continue;
    seen.add(id);
  ...
  state.remoteParticipants = participants.slice(0, maxLiveParticipants - 1);
  state.remoteStream = remoteStreams.values().next().value || null;
...
function attachMediaStreams() {
  ...
  document.querySelectorAll("[data-remote-video]").forEach((video) => {
    const peerId = video.dataset.remoteVideo || "";
...
function createPeerConnection(remoteUserId) {
  ...
  peer.ontrack = (event) => {
    ...
    updateRemoteParticipantsFromPresence();
    state.callStatus = "Connected.";
    render();
    attachMediaStreams();
  };
  peer.onicecandidate = (event) => {
    if (event.candidate) sendCallSignal({ type: "ice", candidate: event.candidate }, remoteUserId);
  };
  peer.onconnectionstatechange = () => {
    if (["failed", "closed", "disconnected"].includes(peer.connectionState)) {
      remoteStreams.delete(remoteUserId);
      peerConnections.delete(remoteUserId);
      offeredPeers.delete(remoteUserId);
      updateRemoteParticipantsFromPresence();
      render();
      attachMediaStreams();
    }
...
async function getPeerConnection(remoteUserId) {
  if (!remoteUserId || remoteUserId === state.user?.id) return null;
  if (peerConnections.has(remoteUserId)) return peerConnections.get(remoteUserId);
  if (peerConnections.size >= maxLiveParticipants - 1) {
    state.callStatus = "This room is full. Nakaru-San supports up to 3 cameras in this build.";
    return null;
  }
  const peer = createPeerConnection(remoteUserId);
  peerConnections.set(remoteUserId, peer);
  state.peer = peer;
  state.stream?.getTracks().forEach((track) => peer.addTrack(track, state.stream));
  return peer;
}

async function sendCallSignal(payload, to = "all") {
  if (!state.callChannel || !state.user) return;
  await state.callChannel.send({
    type: "broadcast",
    event: "signal",
    payload: { ...payload, from: state.user.id, to, room: callRoomId() }
  });
}
...
async function handleCallSignal(payload) {
  ...
  try {
    if (payload.type === "offer") {
      ...
      await sendCallSignal({ type: "answer", answer }, payload.from);
      state.callStatus = "Answer sent. Connecting...";
    }
    if (payload.type === "answer") {
      if (peer.signalingState !== "stable") await peer.setRemoteDescription(new RTCSessionDescription(payload.answer));
      await flushPendingIce(payload.from, peer);
    ...
    if (payload.type === "ice" && payload.candidate) {
      if (peer.remoteDescription) {
        ...
        const list = pendingIceCandidates.get(payload.from) || [];
        list.push(payload.candidate);
        pendingIceCandidates.set(payload.from, list);
      }
    }
  } catch (error) {
...
async function flushPendingIce(remoteUserId, peer) {
  const list = pendingIceCandidates.get(remoteUserId) || [];
...
async function createOfferFor(remoteUserId) {
  if (offeredPeers.has(remoteUserId)) return;
  const peer = await getPeerConnection(remoteUserId);
  if (!peer || peer.signalingState !== "stable") return;
  offeredPeers.add(remoteUserId);
  const offer = await peer.createOffer();
  await peer.setLocalDescription(offer);
  await sendCallSignal({ type: "offer", offer }, remoteUserId);
}

async function handleCallPresenceSync() {
  if (!state.callChannel || !state.user || !state.stream) return;
  const participants = updateRemoteParticipantsFromPresence();
  ...
  for (const id of peerConnections.keys()) {
    if (!activeIds.has(id)) {
      ...
      offeredPeers.delete(id);
    }
  }
  const joinable = participants.slice(0, maxLiveParticipants - 1);
  for (const participant of joinable) {
    if (String(state.user.id) < String(participant.id)) {
      await createOfferFor(participant.id);
    } else {
      await getPeerConnection(participant.id);
    }
  }
  state.callStatus = joinable.length
    ? `Connected room with ${joinable.length + 1} participant${joinable.length ? "s" : ""}.`
    : "You are live. Waiting for others to join.";
  render();
  attachMediaStreams();
}
...
async function connectCall(mode = "video", isCaller = false) {
  ...
  try {
    ...
    state.callChannel = supabaseClient.channel(`nakaru-call-${callRoomId()}`, { config: { broadcast: { self: false }, presence: { key: state.user.id } } });
    state.callChannel.on("broadcast", { event: "signal" }, ({ payload }) => handleCallSignal(payload));
    state.callChannel.on("presence", { event: "sync" }, () => handleCallPresenceSync());
    state.callChannel.on("presence", { event: "leave" }, () => handleCallPresenceSync());
...
async function startCall(mode = "video") {
  if (state.page === "live" && (!state.activeLiveRoom || state.activeLiveRoom.host_id !== state.user?.id)) {
...
async function stopCamera(shouldRender = true) {
  ...
  if (state.callChannel && supabaseClient) {
    try {
      await state.callChannel.untrack?.();
...
function renderPost(post) {
  ...
  const commentsOpen = state.commentingPostId === post.id;
  ...
  return `
    <article class="post-card">
      ...
      ${postType === "image" && post.media_url ? `<figure class="post-media"><img class="post-image" src="${escapeHtml(post.media_url)}" alt="Nakaru-San post media" loading="lazy" /></figure>` : ""}
      ...
      <div class="post-actions">
        <button class="${liked ? "active" : ""}" onclick="toggleLikePost('${post.id}')" type="button">${liked ? "Liked" : "Like"} · ${post.likes || 0}</button>
        <button class="${commentsOpen ? "active" : ""}" onclick="toggleComments('${post.id}')" type="button">Comment · ${post.comments_count || post.comments || comments.length || 0}</button>
        <button onclick="sharePost('${post.id}')" type="button">Share</button>
      </div>
      ${commentsOpen ? `
        <div class="comment-panel">
          <div class="comment-list">${comments.length ? comments.map((comment) => `<div class="comment-item"><strong>${escapeHtml(comment.author || profileName(profileById(comment.user_id)))}</strong><p>${escapeHtml(comment.content || "")}</p></div>`).join("") : `<p class="empty-state">No comments yet. Start the conversation.</p>`}</div>
          <form class="comment-form" onsubmit="submitPostComment(event, '${post.id}')">
            <input value="${escapeHtml(state.commentDrafts[post.id] || "")}" oninput="updateCommentDraft('${post.id}', this.value)" placeholder="Write a comment" />
            <button class="primary-action" type="submit">Post</button>
          </form>
        </div>
      ` : ""}
    </article>
...
function renderLiveVideoGrid(context = "live") {
  const remoteSlots = state.remoteParticipants.slice(0, maxLiveParticipants - 1);
  while (remoteSlots.length < maxLiveParticipants - 1) {
    remoteSlots.push(null);
  }
  const localLabel = state.user ? profileName({ ...state.profile, id: state.user.id }) : "You";
  return `
    <div class="live-grid trio">
      <div class="live-stage">
        ${state.stream ? `<video id="live-video" autoplay muted playsinline></video><span class="video-name">${escapeHtml(localLabel)} ${context === "live" ? "(host/local)" : "(you)"}</span>` : `<div><strong>Your camera preview appears here.</strong><span>${context === "live" ? "Create or join a room to connect with up to 3 people." : "Start or accept a call to connect."}</span></div>`}
      </div>
      ${remoteSlots.map((participant, index) => participant?.id
        ? `<div class="live-stage remote-stage"><video data-remote-video="${escapeHtml(participant.id)}" autoplay playsinline></video><span class="video-name">${escapeHtml(profileName(participant))}</span></div>`
        : `<div class="live-stage remote-stage"><div><strong>${context === "live" ? `Guest ${index + 1}` : "Friend video"}</strong><span>${context === "live" ? "Waiting for someone to join this live room." : "Your friend appears here after they accept and connect."}</span></div></div>`
      ).join("")}
    </div>
  `;
}
...
function renderLiveNowStrip() {
  const rooms = state.liveRooms.slice(0, 4);
  return `
    <section class="live-now-strip">
      <div class="panel-title inline"><div><span class="eyebrow">Live now</span><h2>Join active live rooms</h2></div><button class="ghost-action" onclick="setPage('golive')" type="button">Open Live Directory</button></div>
      <div class="user-card-list">${rooms.length ? rooms.map(liveRoomCard).join("") : `<p class="empty-state">No one is live yet. Start a room from GoLive.</p>`}</div>
    </section>
  `;
}

function renderPostComposer(placeholder = "Share an anime theory, gaming update, or watch-party plan") {
  if (!state.user) {
    return `<div class="composer locked-composer">${avatar(state.profile)}<p class="empty-state">Sign in to post text, images, or videos.</p><button class="primary-action" onclick="setPage('edit-profile')" type="button">Sign In</button></div>`;
  }
  return `
    <form class="composer media-post-composer" onsubmit="createTextPost(event)">
      ${avatar(state.profile)}
      <textarea name="postText" rows="2" placeholder="${escapeHtml(placeholder)}"></textarea>
      <label class="attach-button">Media<input type="file" accept="image/*,video/*" /></label>
      <button class="primary-action" ${state.postCreating ? "disabled" : ""} type="submit">${state.postCreating ? "Posting..." : "Post"}</button>
    </form>
  `;
}
...
function renderPage() {
  ...
  if (state.page === "feed") return `
    <main class="content-layout">
      <section class="panel">
        ...
        ${renderLiveNowStrip()}
        ${renderPostComposer("Share text, images, clips, anime theories, or gaming updates")}
        ${state.postStatus ? `<p class="status-text">${escapeHtml(state.postStatus)}</p>` : ""}
        ${state.postActionStatus ? `<p class="status-text">${escapeHtml(state.postActionStatus)}</p>` : ""}
  ...
  `;
  if (state.page === "profile") return `<main class="content-layout">${profileCard(false)}<section class="panel"><div class="panel-title"><span class="eyebrow">Profile feed</span><h2>Posts by ${escapeHtml(state.profile.display_name || state.profile.username)}</h2></div>${renderPostComposer("Post text, images, or videos to your profile")}${state.postStatus ? `<p class="status-text">${escapeHtml(state.postStatus)}</p>` : ""}${state.postActionStatus ? `<p class="status-text">${escapeHtml(state.postActionStatus)}</p>` : ""}<div class="feed-list">${profilePosts.length ? profilePosts.map(renderPost).join("") : `<p class="empty-state">No posts yet.</p>`}</div></section></main>`;
      ...
      <main class="content-layout">
        <section class="panel">
          ...
          <div class="user-card-list">${incomingPending.length ? incomingPending.map((request) => {
            const profile = profileById(request.sender_id) || { id: request.sender_id, username: "member", display_name: "Nakaru Member" };
        ...
        <aside class="panel sidebar-panel">
          ...
          <div class="user-card-list">${outgoingPending.length ? outgoingPending.map((request) => {
            const profile = profileById(request.receiver_id) || { id: request.receiver_id, username: "member", display_name: "Nakaru Member" };
  ...
  if (state.page === "inbox" || state.page === "messages") {
    const messageTargets = state.friends.filter((profile) => profile.id !== state.user?.id);
    const activeRecipient = state.friends.find((profile) => profile.id === state.activeDmRecipient);
    const dmMessages = activeRecipient?.id ? state.dmMessages : [];
    return `
      <main class="inbox-layout">
        <section class="panel thread-list">
          ...
          ${messageTargets.length ? messageTargets.map((profile) => `<div class="thread ${profile.id === state.activeDmRecipient ? "active" : ""}">${profileIdentityButton(profile)}<button class="ghost-action" onclick="setDmRecipient('${profile.id}')" type="button">Chat</button></div>`).join("") : `<p class="empty-state">Accepted friends will appear here. Send or accept a friend request first.</p>`}
        </section>
        <section class="panel dm-panel">
          <div class="panel-title"><span class="eyebrow">Conversation</span><h2>${activeRecipient?.id ? `<button class="inline-profile-link" onclick="openUserProfile('${activeRecipient.id}')" type="button">${escapeHtml(activeRecipient.display_name || activeRecipient.username)}</button>` : "Choose a friend"}</h2></div>
          <div class="dm-window">${dmMessages.map(renderDmBubble).join("")}</div>
          ${activeRecipient?.id ? `<form class="message-form media-message-form" onsubmit="sendDm(event)"><input placeholder="Message ${escapeHtml(activeRecipient.display_name || activeRecipient.username)}" /><label class="attach-button">Media<input type="file" accept="image/*,video/*" /></label><button class="primary-action" type="submit">Send</button></form>` : `<p class="empty-state">Accept a friend request or choose a friend before sending private messages.</p>`}
...
function renderKanjiRain() {
  return `
    <div class="kanji-rain" aria-hidden="true">
      ${kanjiWordRainItems.map(([char, left, duration, delay, tone]) => `<span class="${escapeHtml(tone)}" style="--x:${left}%; --duration:${duration}s; --delay:-${delay}s;">${escapeHtml(char)}</span>`).join("")}
    </div>
  `;
}

function renderBlossomRain() {
  return `
    <div class="blossom-rain" aria-hidden="true">
      ${blossomRainItems.map(([left, duration, delay, size], index) => `<span class="blossom blossom-${index % 4}" style="--x:${left}%; --duration:${duration}s; --delay:-${delay}s; --size:${size};"></span>`).join("")}
    </div>
  `;
}
  ...
  return `
    ...
    <aside class="logo-sidebar ${state.sidebarOpen ? "open" : ""}" aria-label="Nakaru-San menu">
      ...
      <div class="sidebar-cta">
        ...
        ${state.musicStatus ? `<small class="install-hint">${escapeHtml(state.musicStatus)}</small>` : ""}
        ${state.notificationStatus ? `<small class="install-hint">${escapeHtml(state.notificationStatus)}</small>` : ""}
      </div>
    </aside>
  `;
}
...
function renderNotificationTray() {
  ...
  return `
    <section class="notification-tray" aria-label="Nakaru-San notifications">
      <div class="panel-title inline"><div><span class="eyebrow">Notifications</span><h2>Messages and requests</h2></div><button class="ghost-action" onclick="toggleNotifications()" type="button">Close</button></div>
      ${state.notificationStatus ? `<p class="status-text">${escapeHtml(state.notificationStatus)}</p>` : ""}
      <div class="notification-list">${state.notifications.length ? state.notifications.map((item) => `
        <button class="notification-item" onclick="openNotification('${escapeHtml(item.type)}')" type="button">
          <strong>${escapeHtml(notificationTitle(item.type))}</strong>
          <span>${escapeHtml(item.text)}</span>
          <small>${escapeHtml(formatTime(item.created_at))}</small>
        </button>
      `).join("") : `<p class="empty-state">No notifications yet. New messages, requests, calls, and live invites will appear here.</p>`}</div>
    </section>
  `;
}
...
function render() {
  ...
  const pendingRequestCount = state.incomingRequests.filter((request) => request.status === "pending").length;
  ...
  const nav = [
    ...
    ["messages", "Messaging"],
    ["calls", "Calls"],
    ["friend-requests", pendingRequestCount ? `Requests (${pendingRequestCount})` : "Requests"],
    ["inbox", "Inbox"]
  ];
...
window.updateCommentDraft = updateCommentDraft;
