...
async function resendVerificationEmail(event) {
  ...
  render();
  try {
    const { error } = await withTimeout(supabaseClient.auth.resend({
      type: "signup",
      email,
      options: { emailRedirectTo: redirectUrl() }
    }), "Confirmation email", 8000);
    if (error) throw error;
...
async function saveProfile() {
  ...
  try {
    if (supabaseClient) {
      let { error } = await withTimeout(supabaseClient.from("profiles").upsert(row, { onConflict: "id" }), "Profile save", 9000);
      if (error && isMissingColumnError(error, "banner_url")) {
        const { banner_url, ...rowWithoutBanner } = row;
        ({ error } = await withTimeout(supabaseClient.from("profiles").upsert(rowWithoutBanner, { onConflict: "id" }), "Profile save retry", 9000));
      }
      if (error && String(error.message || "").toLowerCase().includes("duplicate")) {
        row.username = `${row.username}_${suffix}`.slice(0, 31);
        const { banner_url, ...retryRow } = row;
        ({ error } = await withTimeout(supabaseClient.from("profiles").upsert(retryRow, { onConflict: "id" }), "Profile username retry", 9000));
      }
      if (error) throw error;
    } else {
      const profiles = readLocal("nakaru-local-profiles", {});
