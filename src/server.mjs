...
const appBaseUrl = process.env.APP_BASE_URL || `http://${process.env.HOST || "127.0.0.1"}:${process.env.PORT || 4173}`;
...
function handleApi(request, response, url) {
  ...
  if (url.pathname.match(/^\/api\/auth\/oauth\/[^/]+$/) && request.method === "GET") {
    ...
    const config = oauthProviders[provider];
    if (!config) return json(response, 404, { error: "Unknown OAuth provider." });
