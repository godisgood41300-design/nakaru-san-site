...
function upsertOAuthAccount(provider, profile) {
  ...
  const account = { id: randomUUID(), username, email, passwordHash: "", provider, providerId };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (route.match(/^\/auth\/oauth\/[^/]+\/callback$/) && method === "GET") {
      ...
      const sessionId = randomUUID();
      store.sessions.set(sessionId, account.id);
      res.setHeader("Set-Cookie", `nakaru_session=${sessionId}; HttpOnly; SameSite=Lax; Path=/; Max-Age=2592000`);
