...
async function chatApi(path, options = {}) {
  if (!useSupabaseDirect) throw new Error("Supabase chat is not configured");
...
function notifyChatUnavailable() {
  if (chatFailureNotified) return;
  chatFailureNotified = true;
  showToast("Chat unavailable", "Chat is temporarily unavailable. Please try again soon.", { dedupeMs: 60000 });
}
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      ...
      if (providerStatus && !providerStatus.configured) {
        ...
        authMessage.textContent = `${provider} login needs ${missing} in your hosting settings first.`;
        showToast("Social login setup needed", `Add ${missing} before using ${provider} login.`);
        return;
      }
    } catch {
