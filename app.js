...
async function loadFriendships() {
  ...
  if (!activeRecipientIsFriend) state.activeDmRecipient = state.friends[0]?.id || "";
}
...
function renderPage() {
  ...
  `;
  if (state.page === "profile") return `<main class="content-layout">${profileCard(false)}<section class="panel"><div class="panel-title"><span class="eyebrow">Profile feed</span><h2>Posts by ${escapeHtml(state.profile.display_name || state.profile.username)}</h2></div>${state.postActionStatus ? `<p class="status-text">${escapeHtml(state.postActionStatus)}</p>` : ""}<div class="feed-list">${profilePosts.length ? profilePosts.map(renderPost).join("") : `<p class="empty-state">No posts yet.</p>`}</div></section></main>`;
